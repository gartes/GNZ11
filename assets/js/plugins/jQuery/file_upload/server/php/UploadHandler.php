...
class UploadHandler
{
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            ...
            'mkdir_mode' => 0755,
            'param_name' => 'files',
            // Установите следующую опцию на «POST», если ваш сервер не поддерживает
            // DELETE запросы. Этот параметр отправляется клиенту:
            'delete_type' => 'DELETE',
            ...
            // Включите, чтобы обеспечить загрузку файлов через GET-запросы к PHP-скрипту:
            //     1. Установите 1 для загрузки файлов методом readfile через PHP
            //     2. Установите 2 для отправки заголовка X-Sendfile для lighttpd / Apache
            //     3. Установите 3 для отправки заголовка X-Accel-Redirect для nginx
            // Если установлено значение 2 или 3, настройте параметр upload_url на базовый путь
            // параметр перенаправления, e.g. '/files/'.
	        ...
	        // Настройки php.ini upload_max_filesize и post_max_size
	        // имеет приоритет над следующей настройкой max_file_size:
            'max_file_size' => null,
            'min_file_size' => 1,
            // Максимальное количество файлов для каталога загрузки:
            'max_number_of_files' => null,
            // Читает первые байты файла, чтобы определить и исправить расширения файла:
            ...
            'min_height' => 1,
            // 62/5000
	        //Установите для следующего параметра значение false, чтобы включить возобновляемую загрузку:
            'discard_aborted_uploads' => true,
            // Set to 0 to use the GD library to scale and orient images,
            // set to 1 to use imagick (if installed, falls back to GD),
            // set to 2 to use the ImageMagick convert binary directly:
            'image_library' => 1,
            // Раскомментируйте следующее, чтобы определить массив ограничений ресурсов
            // for imagick:
            /*
            ...
            */
            // Команда или путь к двоичному преобразователю ImageMagick:
            'convert_bin' => 'convert',
            ...
            */
            ...
            'image_versions' => array(
	            ...
	            // Вы можете добавить массивы для генерации разных версий.
	            // Имя ключа - это название версии (пример: 'medium').
	            // массив содержит опции для применения.
                /*
                'medium' => array(
                    'max_width' => 800,
                    'max_height' => 600
                ...
                */
                'thumbnail' => array(
	                // Раскомментируем следующее, чтобы использовать определенный каталог для миниатюр
	                // вместо подкаталога на основе идентификатора версии.
	                // Убедитесь, что этот каталог не разрешает выполнение файлов, если вы
	                // не накладываем никаких ограничений на тип загружаемых файлов, например по
	                // копируем файл .htaccess из каталога файлов для Apache:
//                    'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/thumb/',
//                    'upload_url' => $this->get_full_url().'/thumb/',
                    // Раскомментируйте следующее, чтобы вызвать максимум
                    // размеры и, например, создать квадратные миниатюры::
                    // 'auto_orient' => true,
                    // 'crop' => true,
                    // 'jpeg_quality' => 70,
                    // 'no_cache' => true, (есть опция кэширования, но она запоминает размеры миниатюр от предыдущего действия!)
                    // 'strip' => true, (это удаляет теги EXIF, такие как геолокация)
                    'max_width' => 80, // либо укажите ширину, либо установите на 0. Затем ширина будет автоматически отрегулирована, сохраняя соотношение сторон до заданного значения max_height.
                    'max_height' => 80 // либо задайте высоту, либо установите на 0. Затем высота автоматически регулируется, сохраняя соотношение сторон до заданной max_width.
                )
            ),
            'print_response' => true
        );
...
//        echo'<pre>';print_r( $this->options );echo'</pre>'.__FILE__.' '.__LINE__;
//        die(__FILE__ .' '. __LINE__ );
    ...
    protected function get_upload_path($file_name = null, $version = null) {
        ...
        $upload_dir = $this->options['upload_dir'].$this->get_user_path()
...
//		die(__FILE__ .' '. __LINE__ );
	...
	/**
	 * Заглушка для обработки информации от формы
	 * @param $file
	 * @param $index
	 * @since 3.9
	 */
...
//        die(__FILE__ .' '. __LINE__ );
	...
	/**
	 * Создание ответа для загруженных файлов
	 * @param bool $print_response печать ответа в body
	 * @since 3.9
	 */
